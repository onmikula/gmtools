...
col.links=1, col.bg="lightgrey", lwd.links=1, lwd.bg=1, ...) {
	...
	if (add == FALSE) {
		...
		if (!is.null(userMatrix)) {
			rgl::view3d(userMatrix=userMatrix, fov=view[3], zoom=view[4], scale=rgl::par3d("scale"), interactive=TRUE, type=c("userviewpoint", "modelviewpoint"))
		} else {
			rgl::view3d(theta=view[1], phi=view[2], fov=view[3], zoom=view[4], scale=rgl::par3d("scale"), interactive=TRUE, type=c("userviewpoint", "modelviewpoint"))
		}
	}
